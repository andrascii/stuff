...
template <typename T>
T GetOptionValue(const cxxopts::ParseResult& result,
...
Expected<std::shared_ptr<CommandLineArguments>>
CommandLineArguments::Read(int argc, char** argv, const std::string& app_name, const std::string& app_description) {
  struct MakeSharedEnabler : public CommandLineArguments {
    MakeSharedEnabler(std::filesystem::path json_config_path,
                      bool help_requested,
                      std::string help,
  ...
  try {
    ...
    options.add_options()(kConfigOption, kConfigOptionDescription, cxxopts::value<std::filesystem::path>())(
      kHelpOption,
      kHelpOptionDescription);

    options.allow_unrecognised_options();

    const auto result = options.parse(argc, argv);
    const auto help_requested = result.count(kHelpOption) > 0;
    const auto json_config_path =
      GetOptionValue<std::filesystem::path>(result, kConfigOption);
...
std::filesystem::path CommandLineArguments::ConfigPath() const noexcept {
  return json_config_path_;
}
...
const std::string& CommandLineArguments::Help() const { return help_; }

const std::vector<std::string>&
CommandLineArguments::Unmatched() const noexcept {
  return unmatched_;
}
