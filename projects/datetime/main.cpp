...
TimePoint<Milliseconds> ConvertToTimeZone(const TimePoint<Milliseconds>& time_point, const std::string& time_zone) {
  const auto zoned_time_point = date::make_zoned(time_zone, time_point);
  return time_point + zoned_time_point.get_info().offset;
}

TimePoint<Milliseconds> ZonedStartOfTheCurrentDay(const std::string& time_zone) {
  const auto gmt_start_of_the_current_day = GmtStartOfTheCurrentDay();
  const auto zoned_time_point = date::make_zoned(time_zone, gmt_start_of_the_current_day);
...
int main() {
  ...
  const auto start_of_the_day_current_day_gmt_to_msk_converted = ConvertToTimeZone(start_of_the_day_current_day, "Europe/Moscow");
  const auto start_of_the_day_current_day_by_msk = ZonedStartOfTheCurrentDay("Europe/Moscow");

  const auto start_of_the_day_current_day_gmt_to_arg_converted = ConvertToTimeZone(start_of_the_day_current_day, "America/Argentina/ComodRivadavia");
  const auto start_of_the_day_current_day_by_arg = ZonedStartOfTheCurrentDay("America/Argentina/ComodRivadavia");

  std::cout
    << "start of the day by GMT: "
    << start_of_the_day_current_day
    << std::endl;

  std::cout
    << "start of the day by GMT (MSK time at this moment): "
    << start_of_the_day_current_day_gmt_to_msk_converted
    << std::endl;

  std::cout
    << "start of the day by MSK (MSK to GMT corrected): "
    << start_of_the_day_current_day_by_msk
    << std::endl;
  ...
  std::cout
    ...
    << start_of_the_day_current_day_gmt_to_arg_converted
    << std::endl;

  std::cout
    << "start of the day by GMT (Argentina to GMT corrected): "
    << start_of_the_day_current_day_by_arg
    << std::endl;
